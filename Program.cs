...
_ = Task.Run(Tools.Strings.GetEmojis);
_ = Task.Run(Tools.Strings.GetLanguages);
...
if (!dev)
{
    clusters = await Tools.Railway.getReplicaCount();
}
...
while (pubSub is null)
{
    ...
    if (pubSub is null) Thread.Sleep(500);
}
...
_ = Task.Run(() =>
{
    ...
    Thread.Sleep(5000); // Make sure no overlap with other clusters
    ...
    else
    {
        ...
        {
            pubSub.Publish("start", $"{message.ToString()}:1");
